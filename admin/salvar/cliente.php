	...
	if ( count ($data_nascimento) == 3) {
		...
		if ($data_nascimento <= 7) {
			...
			if(checkdate($mes, $dia, $ano)){
				...
				$Validadata_nascimento =  $Validadata_nascimento[0]."-".$Validadata_nascimento[1]."-".$Validadata_nascimento[2];
			...
			if ($Validadata_nascimento <= "1930-01-01") {

				echo '<script>alert("A data está abaixo do permitido");history.back();</script>';

				} else {
					
					if ($Validadata_nascimento >= "2001-01-01") {

						echo '<script>alert("O Cliente precisa ser maior de idade");history.back();</script>';
					...
					if ($consulta->execute()) {
						...
						if ($_SESSION["sistema"]["idtipousuario"] == 1) {
							...
							echo "<script>
									...
									setTimeout(() => { 
										location.href='homefuncionario.php?op=listar&pg=cliente';
									}, 1000);
								</script>";								
							exit;
						}
